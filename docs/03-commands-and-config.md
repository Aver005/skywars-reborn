...
| `/sw kits` | GUI глобальных стартовых наборов (создать/редактировать/дефолт/удалить) |
| `/sw loot` | GUI глобальных категорий лута сундуков |
...
**SkyWars-специфика:** киты и категории лута — глобальные (`/sw kits`, `/sw loot`;
файлы `plugins/SkyWars/kits.yml` и `loot.yml`). Точки-сундуки арены — в `/sw gui <ID>`
→ Точки → Сундуки (выбрать категорию → маркер → ПКМ по блоку). Числа игры
`capsule-seconds` / `grace-seconds` — `/sw gui <ID>` → Параметры (2-й ряд); `/sw set`
их пока не принимает (только общие числа каркаса). Правила и цепочка — [04-skywars.md](04-skywars.md).
